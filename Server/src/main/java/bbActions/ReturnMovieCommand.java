...
public class ReturnMovieCommand extends AbstractBlockBusterCommand {

	private String _movie_name;

	public ReturnMovieCommand(String[] arguments_to_translate) {
		try {
	...
	public void runCommand() {
		if (_movie_name == null || !_myBB_protocol.get_logged_in().get()) {
			AbstractUserServiceCommand error = new ErrorCommand("request return failed");
			_myBB_protocol.get_connections().send(_myBB_protocol.get_connectionID(), error);
		}
		else {
			String toBroadcast = _myBB_protocol.get_shared_data().return_movie(_myBB_protocol.get_my_user_name(), _movie_name);
			if(!"".equals(toBroadcast)) {
				AbstractUserServiceCommand ack = new AckCommand("return \"" + _movie_name + "\" success");
