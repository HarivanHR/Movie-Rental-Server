...
public class ActorThreadPool {
    ...
    public ActorThreadPool(int threads) {
        this.threads = Executors.newFixedThreadPool(threads); //Create a new
        acts = new WeakHashMap<>();
        playingNow = ConcurrentHashMap.newKeySet();
        actsRWLock = new ReentrantReadWriteLock();
    }

    public void submit(Object act, Runnable task_to_perform) {
        synchronized (act) {
            if (!playingNow.contains(act)) {
                playingNow.add(act);
                execute(task_to_perform, act);
            } else {
                pendingRunnablesOf(act).add(task_to_perform);
            }
        }
    }
    ...
    private void execute(Runnable task_to_perform, Object act) {
        threads.execute(() -> {
            try {
            	task_to_perform.run();
            } finally {
